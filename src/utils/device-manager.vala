...
namespace DeviceManager{
    ...
    public class DeviceProber {
        ...
        public static string ? stringify_device_technology (
            uint? device_technology) {
        ...
        public static string ? create_health_description (double? capacity) {
            ...
            if (capacity >= 95)
                return _(
                    "The device is near or at its maximum rated capacity. It is in excellent condition and should not require much intervention.");
            else if (capacity >= 90)
                return _(
                    "The device performs close to its original capacity. There is little noticeable difference from its optimal state.");
            else if (capacity >= 80)
                return _(
                    "The device has lost some capacity, but it should not be of much concern. Continue to take precautions regarding your power device like limiting its charge and using power-saving settings.");
            else if (capacity >= 70)
                return _(
                    "The device has noticeably degraded in capacity, but is still usable. Runtime may be shorter than at its original capacity. Use power-optimizing settings to extend longevity and slow down degradation.");
            else if (capacity >= 60)
                return _(
                    "The device has experienced a significant drop in capacity. Mobility can be more difficult due to a decrease in runtime. Depending on usage habits, replacement may be necessary in the future.");
            else if (capacity > 0)
                return _(
                    "The device has undergone substantial deterioration. Power instability and potential overheating can damage other components. Replace the device to avoid further damage.");
        ...
        public DeviceObject fetch_device (
            ObjectPath object_path) throws IOError {
            ...
            device.device_type =
                DeviceProber.stringify_device_type (upower_proxy.device_type);
            device.technology =
                DeviceProber.stringify_device_technology (
                upower_proxy.technology);
            device.state =
                DeviceProber.stringify_device_state (upower_proxy.state);
            ...
            device.energy_full_design =
                positive_double (upower_proxy.energy_full_design);
            ...
            device.voltage_min_design =
                positive_double (upower_proxy.voltage_min_design);
            ...
            if (upower_proxy.charge_threshold_enabled &&
                ...
                device.charge_start_threshold =
                    positive_uint32 (upower_proxy.charge_start_threshold);
                device.charge_end_threshold =
                    positive_uint32 (upower_proxy.charge_end_threshold);
            }
            ...
            device.health_description =
                DeviceProber.create_health_description (device.capacity);
